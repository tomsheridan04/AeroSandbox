...
CHORD_TWIST_OPTIMIZATION_OUTPUTS = (
    OUTPUTS / "chord_twist_optimization_free_chord_direct_root_low_j"
)
...
N_CHORD_CONTROL_POINTS = 6
N_TWIST_CONTROL_POINTS = 6
SPLINE_DEGREE = 3
OPTIMIZATION_RADIAL_RESOLUTION = 10
REPORT_RADIAL_RESOLUTION = 24
CHECK_RADIAL_RESOLUTION = 32
THRUST_MATCH_FACTOR = 1.0
THRUST_TARGET_MARGIN_N = 0.12
MIN_CHORD_M = 1e-4
MAX_CHORD_M = 0.12
MIN_THICKNESS_RATIO = 0.055
MAX_THICKNESS_RATIO = 0.42
...
def make_bspline_function(control_points, knots: onp.ndarray):
...
def make_chord_twist_optimized_propeller(
    propeller_apc: asb.Propeller,
    chord_control_points,
    twist_control_points,
    knots: onp.ndarray,
) -> asb.Propeller:
    # B-spline chord and twist are supplied as callable inputs. The Propeller
    # object itself does not natively build a B-spline distribution here.
    chord_function = make_bspline_function(
        control_points=chord_control_points,
        knots=knots,
    )
    twist_function = make_bspline_function(
        control_points=twist_control_points,
        knots=knots,
    )

    def thickness_function(r_over_R):
        baseline_absolute_thickness = propeller_apc.chord(
            r_over_R
        ) * propeller_apc.thickness(r_over_R)
        return baseline_absolute_thickness / chord_function(r_over_R)

    return asb.Propeller(
        name="APC 12x8E direct + root, chord+twist optimized",
        ...
        chord_distribution=chord_function,
        twist_distribution=twist_function,
...
def plot_geometry(
    ...
    axes[3].plot(
        geometry_table["r_over_R"],
        geometry_table["absolute_thickness_apc_m"] / inch,
        color="black",
        linewidth=2,
        label="APC",
    )
    axes[3].plot(
        geometry_table["r_over_R"],
        geometry_table["absolute_thickness_optimized_m"] / inch,
        color="#1f77b4",
        linestyle="--",
        linewidth=2,
        label="Optimized",
    )
    axes[3].set_ylabel("Absolute thickness [in]")
    ...
    fig.suptitle(
        "Direct + Root Chord+Twist Optimization "
...
def run_chord_twist_optimization() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    CHORD_TWIST_OPTIMIZATION_OUTPUTS.mkdir(parents=True, exist_ok=True)

    propeller_apc = parse_apc_geometry_file()
    r_over_R_apc = onp.asarray(propeller_apc.radial_stations, dtype=float)
    chord_apc = onp.asarray(propeller_apc.chord_distribution, dtype=float)
    ...
    chord_control_points_initial = fit_bspline_control_points(
        x=r_over_R_apc,
        y=chord_apc,
        knots=knots,
        n_control_points=N_CHORD_CONTROL_POINTS,
        degree=SPLINE_DEGREE,
    ...
    chord_control_points = opti.variable(
        init_guess=chord_control_points_initial,
        scale=0.02,
        lower_bound=MIN_CHORD_M,
        upper_bound=MAX_CHORD_M,
    )
    ...
    propeller_optimized_symbolic = make_chord_twist_optimized_propeller(
    ...
    thickness_optimized_grid = propeller_optimized_symbolic.thickness(
        geometry_constraint_r
    )
    ...
    opti.subject_to(
        [
            thickness_optimized_grid >= MIN_THICKNESS_RATIO,
            thickness_optimized_grid <= MAX_THICKNESS_RATIO,
    ...
    twist_regularization = 3e-5 * np.sum(
        ((twist_control_points - twist_control_points_initial) / 10) ** 2
    )
    chord_regularization = 2e-4 * np.sum(
        (
            (chord_control_points - chord_control_points_initial)
            / onp.maximum(chord_control_points_initial, 3e-3)
        )
        ** 2
    )
    opti.minimize(
        optimization_output["power"] / 100
        + twist_regularization
        + chord_regularization
    )
    ...
    chord_control_points_optimized = solution(chord_control_points)
    twist_control_points_optimized = solution(twist_control_points)
    propeller_optimized = make_chord_twist_optimized_propeller(
        propeller_apc=propeller_apc,
        chord_control_points=chord_control_points_optimized,
    ...
    geometry_table = pd.DataFrame(
        {
            ...
            "absolute_thickness_apc_m": onp.asarray(
                propeller_apc.chord(dense_r) * propeller_apc.thickness(dense_r),
                dtype=float,
            ),
            "absolute_thickness_optimized_m": onp.asarray(
                propeller_optimized.chord(dense_r)
                * propeller_optimized.thickness(dense_r),
                dtype=float,
            ),
    ...
    geometry_table["absolute_thickness_delta_m"] = (
        geometry_table["absolute_thickness_optimized_m"]
        - geometry_table["absolute_thickness_apc_m"]
    )
    ...
    control_points = pd.DataFrame(
        {
            ...
            "chord_initial_m": chord_control_points_initial,
            "chord_optimized_m": chord_control_points_optimized,
        ...
        [
            {
                ...
                "n_chord_control_points": N_CHORD_CONTROL_POINTS,
                ...
                "max_abs_absolute_thickness_delta_m": float(
                    onp.max(onp.abs(geometry_table["absolute_thickness_delta_m"]))
                ),
            }
        ]
    )
    ...
    summary.to_csv(
        CHORD_TWIST_OPTIMIZATION_OUTPUTS
        / "apc_12x8e_direct_root_chord_twist_optimization_summary.csv",
        index=False,
    )
    ...
    geometry_table.to_csv(
        CHORD_TWIST_OPTIMIZATION_OUTPUTS
        / "apc_12x8e_direct_root_chord_twist_optimization_geometry.csv",
        index=False,
    )
    write_spanwise_csv(
        output_apc=output_apc_report,
        output_optimized=output_optimized_report,
        filename=CHORD_TWIST_OPTIMIZATION_OUTPUTS
        / "apc_12x8e_direct_root_chord_twist_optimization_spanwise.csv",
    )
    ...
    plot_geometry(
        ...
        filename=CHORD_TWIST_OPTIMIZATION_OUTPUTS
        / "apc_12x8e_direct_root_chord_twist_optimized_geometry.png",
    ...
    plot_twisted_sections(
        ...
        propeller_optimized=propeller_optimized,
        filename=CHORD_TWIST_OPTIMIZATION_OUTPUTS
        / "apc_12x8e_direct_root_chord_twist_optimized_sections.png",
    )
    plot_spanwise_distributions(
        output_apc=output_apc_report,
        output_optimized=output_optimized_report,
        filename=CHORD_TWIST_OPTIMIZATION_OUTPUTS
        / "apc_12x8e_direct_root_chord_twist_optimized_spanwise_distributions.png",
    )
    plot_three_view(
        propeller=propeller_optimized,
        filename=CHORD_TWIST_OPTIMIZATION_OUTPUTS
        / "apc_12x8e_direct_root_chord_twist_optimized_three_view.png",
        title="APC 12x8E Direct + Root Chord+Twist Optimized Three-View",
    )

    print(summary.to_string(index=False))
    print(
        "Wrote chord+twist optimization outputs to: "
        f"{CHORD_TWIST_OPTIMIZATION_OUTPUTS}"
    )
